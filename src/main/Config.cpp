...
Config::load(std::string const& filename)
{
    ...
    try
    {
        ...
        for (auto& item : g)
        {
            ...
            /* *** custom attributes parsing ***  IMMORT_MARKED */
            else if (item.first == "ADMIN_ADDRESS")
            {
                if (!item.second->as<std::string>())
                {
                    throw std::invalid_argument("invalid ADMIN_ADDRESS");
                }
                ADMIN_ADDRESS = item.second->as<std::string>()->value();
            }
            else if (item.first == "BASE_NATIVE_AMOUNT")
            {
                if (!item.second->as<int64_t>())
                {
                    throw std::invalid_argument("invalid BASE_NATIVE_AMOUNT");
                }
                BASE_NATIVE_AMOUNT = (long long int)item.second->as<int64_t>()->value();
            }
            else if (item.first == "BASE_RESERVE")
            {
                ...
                BASE_RESERVE = (int)item.second->as<int64_t>()->value();
            }
            else if (item.first == "BASE_RESERVE_INIT")
            {
                if (!item.second->as<int64_t>())
                {
                    throw std::invalid_argument("invalid BASE_RESERVE_INIT");
                }
                BASE_RESERVE_INIT = (int)item.second->as<int64_t>()->value();
            }
            else if (item.first == "BASE_RESERVE_ONLY")
            {
                if (!item.second->as<bool>())
                {
                    throw std::invalid_argument("invalid BASE_RESERVE_ONLY");
                }
                BASE_RESERVE_ONLY = item.second->as<bool>()->value();
            }
            else if (item.first == "INFLATION_RATE_TRILLIONTHS")
            {
                if (!item.second->as<int64_t>())
                {
                    throw std::invalid_argument("invalid INFLATION_RATE_TRILLIONTHS");
                }
                INFLATION_RATE_TRILLIONTHS = (long long int)item.second->as<int64_t>()->value();
            }
            /* --- end of custom attributes parsing ---*/
