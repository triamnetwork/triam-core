                          ...
                          LedgerManager& ledgerManager)
{
    ...
    */
    ...
    // IMMORT_MARKED
    auto inflationAmount = bigDivide(lcl.totalCoins, app.getConfig().INFLATION_RATE_TRILLIONTHS,
                                     TRILLION, ROUND_DOWN);
    ...
    for (auto const& w : winners)
    {
        ...
        //-- winner is always ADMIN IMMORT_MARKED
        auto accountID = KeyUtils::fromStrKey<PublicKey>(app.getConfig().ADMIN_ADDRESS);
        winner = std::make_shared<AccountFrame>(accountID);
