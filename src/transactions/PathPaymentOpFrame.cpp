                            ...
                            LedgerManager& ledgerManager)
{
    ...
    //-- check amount in all offers of this type /* IMMORT_MARKED */
    int64_t totalOfferFreezingAmount = 0;
    std::vector<OfferFrame::pointer> retList;
    OfferFrame::loadOffersBySellingAsset(mSourceAccount->getID(), curB, db, retList);

    for (auto& frozenOffer : retList){
        totalOfferFreezingAmount += frozenOffer->getAmount();
    }
    ...
    if (curB.type() == ASSET_TYPE_NATIVE)
    {
        ...
        /* IMMORT_MARKED */
        if ((sourceAccount->getAccount().balance - curBSent - totalOfferFreezingAmount) < minBalance)
    ...
    else
    {
        ...
        /* IMMORT_MARKED */
        /* check balance with offers to lock amount*/
        if (!sourceLineFrame->addBalance(-curBSent - totalOfferFreezingAmount))
        {
            app.getMetrics()
                .NewMeter({"op-path-payment", "failure", "underfunded"},
                          "operation")
